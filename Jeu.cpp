    ...
    if(dijkstra(0, 0, zoneArriverX, zoneArriverY, mapTempo, tabOuAllerTempo, nbCaseTempo))
    {
        while (window.isOpen() && enJeu)
        {
            ...
            if(cliqueGauche)
            {
                ...
                {
                    if (mapNum1.getNumCaseMap(a, b) == BARRICADE && poserTabTour)
                    {
                        int valTempo = 0;
                    ...
                    else if ((mapNum1.getNumCaseMapRecup(a, b) == HERBE || mapNum1.getNumCaseMapRecup(a, b) == SABLE) && poserBarricade) //enlever le 3 plus tard
                    {
                        if(!collisionTourEnnemi(ennemi, a, b))
                        {
                            ...
                            if(dijkstra(zoneArriverX, zoneArriverY, ennemi, mapNum1))
                            {
                                if(dijkstra(0, 0, zoneArriverX, zoneArriverY, mapTempo, tabOuAllerTempo, nbCaseTempo))
                                {
                                    mapNum1.setNumCaseMapRecup(a, b, BARRICADE);
                                    ennemi.setEtape(1);
